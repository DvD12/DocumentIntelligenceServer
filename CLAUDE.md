...
**Ingestion write ordering is the transaction strategy:** pure computation → Qdrant upsert → SQLite row LAST. On failure after vector writes, the document's points are deleted. Metadata must never reference chunks that don't exist. Dedup follows three rules keyed on SHA-256 (see `pipeline.ingest`); version replacement is delete-first because v2 may produce fewer chunks. IDs are deterministic end to end: new document ids = `uuid5(content_hash)`, point ids = `uuid5(doc_id:chunk_index)` — retry after any crash (including hard process death, where the compensating delete can't run) overwrites residue instead of orphaning it. Search additionally skips points with no metadata row.
